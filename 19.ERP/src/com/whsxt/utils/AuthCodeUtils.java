...
public class AuthCodeUtils {
	...
	// 图片的宽度
	private static final int IMG_WIDHT = AUTHCODE_LENGTH
			* (SINGLECODE_WIDTH + SINGLECODE_GAP);
	// 图片的高度
	private static final int IMG_HEIGHT = SINGLECODE_HEIGHT + 14;
	...
	private static final Color[] COLORS = { Color.RED, Color.BLACK, Color.BLUE,
			Color.GRAY, Color.GREEN, Color.YELLOW, Color.PINK };
	...
	public static BufferedImage getAuthCodeImg(String code) {
		...
		BufferedImage img = new BufferedImage(IMG_WIDHT, IMG_HEIGHT,
		...
		for (int i = 0; i < code.length(); i++) {
			...
			graphics.drawString(code.charAt(i) + "", i
					* (SINGLECODE_WIDTH + SINGLECODE_GAP) + SINGLECODE_GAP / 2,
					SINGLECODE_HEIGHT);
		}
		...
		for (int i = 0; i < 5; i++) {
			graphics.setColor(COLORS[ran.nextInt(COLORS.length)]);
			graphics.drawLine(ran.nextInt(IMG_WIDHT), ran.nextInt(IMG_HEIGHT),
					ran.nextInt(IMG_WIDHT), ran.nextInt(IMG_HEIGHT));
		}
		return img;
	}
}
